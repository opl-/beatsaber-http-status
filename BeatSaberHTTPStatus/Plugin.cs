	...
	internal class Plugin : ICutScoreBufferDidFinishEvent {
		...
		public async void HandleSongStart() {
			...
			try {
				...
				// Unity sucks. The coordinates of the sprite in its texture atlas are only accessible through the Sprite.uv property (rect always returns x=0,y=0), so we need to convert them back into texture space. Fixes #82.
				cover.ReadPixels(new Rect(
					uv.x * texture.width,
					texture.height - uv.y * texture.height - spriteRect.height,
					spriteRect.width,
					spriteRect.height
				), 0, 0);
				cover.Apply();
